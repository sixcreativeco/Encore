...
struct ItineraryItemEditView: View {
    ...
    private func updateType(from eventTitle: String) {
        ...
        if ["hotel", "check in", "check out"].contains(where: lowercasedTitle.contains) {
            newType = .hotel
        }
        else if ["drive", "transport", "bus", "van"].contains(where: lowercasedTitle.contains) {
            newType = .travel
        }
        else if ["flight", "airport"].contains(where: lowercasedTitle.contains) {
            newType = .flight
        }
        else if ["content", "photo", "video"].contains(where: lowercasedTitle.contains) {
            newType = .content
        }
        else if ["food", "lunch", "dinner"].contains(where: lowercasedTitle.contains) {
            newType = .catering
        }
        else if ["lounge"].contains(where: lowercasedTitle.contains) {
            newType = .lounge
        }
            ...
            try db.collection("itineraryItems").document(itemId).setData(from: item, merge: true) { error in
                DispatchQueue.main.async {
                    ...
                    if let error = error {
                        ...
                        print("✅ Successfully saved itinerary item")
                        self.onSave()
                        self.dismiss()
                    }
                }
